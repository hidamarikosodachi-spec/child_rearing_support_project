...
> ## ⛔ 判断: 自動投稿は**見送り**（CEO決裁 2026-09-26・オーナー了承）
>
> **理由**: Meta for Developers のアプリ作成には **Facebook アカウントが必須**だが、本アカウントは Instagram 単独（FBアカウント無し）。
> 新規に FB アカウントを作る必要があり、さらに **60日ごとのトークン更新**という壊れやすい仕組みが増える。
> 一方で得られるのは **IG 投稿 週1本・5分の自動化＝月20分**。しかも IG は現時点で測定可能な流入ゼロ。
> **月20分のために、止まるリスクのある仕組みを増やさない**（2026-06 に Threads トークン切れで投稿が止まった前例あり）。
>
> **再判断のトリガー**: ①オーナーが既存の Facebook アカウントを使える状態になった ②IG が実際に流入を生んでいると確認できた
> ③投稿頻度が週3本以上になった — のいずれか。
>
> **当面の運用**: 画像とキャプションは私が用意し、**投稿はオーナーが手動**（1回5分）。
> 下記の手順書は、再開すると決めたときのために残す。
